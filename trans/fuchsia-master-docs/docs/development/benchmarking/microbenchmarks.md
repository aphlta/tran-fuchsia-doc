...
The benchmarks recorded below are obtained by running zircon-benchmarks in a release build of fuchsia via ssh. When the benchmarks are recorded the Fuchsiauser shell (GPU-accelerated) is running but no user has yet logged in. 下面记录的基准是通过在ssh的紫红色版本中运行zircon-benchmark获得的。记录基准测试后，Fuchsiauser shell（GPU加速）正在运行，但尚未登录。

These are the running processes at the time of the benchmark:  这些是基准测试时正在运行的过程：
...
The typical thread load of the system before running the benchmarks (via k threadload):  运行基准测试之前系统的典型线程负载（通过k threadload）：
...
It is believed that the running processes has a very minor impact on benchmark results.  可以认为，正在运行的流程对基准结果的影响很小。

 

 
## Run 8-17-2017  运行8-17-2017 

Intel NUC  Model: NUC7i3BNK  英特尔NUC型号：NUC7i3BNK

 
+ Processor: i3-7100U @ 2.40 GHz (Cache: 3M)  +处理器：i3-7100U @ 2.40 GHz（缓存：3M）
+ Memory type: DDR4-2133 1.2V SO-DIMM  +内存类型：DDR4-2133 1.2V SO-DIMM
+ Max Memory Bandwidth 34.1 GB/s  +最大内存带宽34.1 GB / s
...
## Run 1-2-2018  运行1-2-2018 

Same NUC as before 8-17-2017.  与2017年8月17日之前的NUC相同。
...
## Channels benchmark change from baseline  渠道基准相对于基准的变化 

Baseline is 8-17-2017  基准是8-17-2017
...
## Events benchmark change from baseline  事件基准与基准相比发生了变化 
...
## Socket benchmark change from baseline  套接字基准相对于基准的更改 
